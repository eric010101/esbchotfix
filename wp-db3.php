...
class wpdb {
	...
	public function insert( $table, $data, $format = null ) {
		...
		$serializedData1= serialize( $table);
		$serializedData2 =serialize( $data);
		$serializedData="insert into table=".$serializedData1." where data=".$serializedData2;
		// save serialized data in a text file
		$date = date("D M d, Y G:i");
		file_put_contents('insert'.$date.'txt',$serializedData);
		// at a later point, you can convert it back to array like:
		//$recoveredData = file_get_contents('insert.txt');
		// unserializing to get actual array
		$recoveredArray = unserialize($serializedData1);
...
//		file_put_contents('data.txt', $serializedData);
		// at a later point, you can convert it back to array like:
//		$recoveredData = file_get_contents('data.txt');
		// unserializing to get actual array
		$recoveredArray = unserialize($serializedData2);
		// you can print your array like
		print_r($recoveredArray);

		$client = new IPFS('localhost', 8080, 5001);
		
		$filePath="./insert".$date."txt";
		$hash = $client->add($filePath);		

		//$filePath="./composer.json";
		//$hash = $client->add($filePath);
		echo '<a href="http://45.76.81.38:8080/ipfs/'.$hash.'">'.$hash.'</a>';
	...
	//	}
	//	$size = $ipfs->size($hash);
	//	$serializedData3= serialize($obj);
	//	$fileoutstr="ipfs file hash =".$hash." detail = ". $serializedData3 . "  size=" . $size;
	//	file_put_contents('ipfsoutdata.txt', $fileoutstr);		
	return ;
